...
class Tipos extends MX_Controller {
    public function __construct(){
        ...
        //Local Models
        $this->models = $this->load->model('tipos_model');
        $this->modelsProyecto = $this->load->model('proyectos_model');
        $this->modelsCategoria = $this->load->model('categorias_model');
    }
        ...
        if (tiene_logeo()) {
            ...
            if ($xValQuery) {
                ...
                if (in_array($xValCi, $xValMeNu)) {
                    $items = array(
                        'getAllTipos'  => $this->models->getAll(),
                        'getAllProyectos'  => $this->modelsProyecto->getAll(),
                        'getAllCategoria'  => $this->modelsCategoria->getAll(),
                    );
                    $this->load->view('tipos',$items);
                }else{
